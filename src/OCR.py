...
class PLCConnection:
    ...
    def send_data(self, int_value, bool_value):
        try:
            data = bytearray(2)
            ...
            logging.info(f"Boolean bit set to {bool_value}.")

        except Exception as e:
            logging.error(f"PLC communication error: {e}")
...
class OCR:
    ...
    def send_PLC(self, int_value):
        if self.plc_connection:
            self.plc_connection.send_data(int_value, True)
